    ...
    static class ClosedTask implements Runnable {
        ...
        public void run() {
            try{
            	...
            	String cId = String.valueOf(session.getAttribute(Cnst.USER_SESSION_CID));
            	if (userId!=null&&userId.matches(userMatcher)) {
                	RedisUtil.hdel(Cnst.get_REDIS_HEART_PREFIX(cId), userId);//删除心跳
                	Player cp = RedisUtil.getPlayerByUserId(userId,cId);
                	if (cp!=null&&cp.getRoomId()!=null) {
                		List<Player> players = RedisUtil.getPlayerList(cp.getRoomId(),cId);
                		List<Long> userIds = new ArrayList<Long>();
						userIds.add(cp.getUserId());
						List<Integer> status = new ArrayList<Integer>();
						status.add(Cnst.PLAYER_LINE_STATE_OUT);
						MessageFunctions.interface_100109(players, status,userIds,2,cId);
					}
				}
            	session.close(true);
            }catch (Exception e){
